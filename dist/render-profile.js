...
export function createRenderProfile(renderer) {
  const originalRender=renderer.render, originalShadow=renderer.shadowMap.render;
  renderer.info.autoReset=false;
  let active=false,depth=0,frame=null,last=null,samples=[];
  ...
  renderer.render=function(...args){
    ...
    try{return originalRender.apply(this,args);}finally{
      ...
      add(nested?frame.reflection:frame.main,before,performance.now()-start,exclude);
    }
  };
  return {
    begin(){renderer.info.reset();frame={main:empty(),reflection:empty(),shadow:empty()};active=true;},
    ...
    getStats(){
      ...
      for(const pass of ['main','reflection','shadow']){
